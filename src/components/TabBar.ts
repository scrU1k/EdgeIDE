...
export class TabBar {
  ...
  constructor(parent: HTMLElement, vfs: VirtualFileSystem, onSelectFile: (id: string) => void) {
    ...
    this.container.className = 'flex items-center bg-[#000000] px-2 py-1 gap-1 overflow-x-auto select-none shrink-0 scrollbar-none';
    ...
    this.container.innerHTML = openTabs.map(tabId => {
      ...
      return `
        <div data-tab-id="${file.id}" 
          ...
          class="tab-item flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-mono cursor-pointer shrink-0 transition-all ${
            isActive 
              ? 'font-semibold shadow-sm' 
              : 'text-zinc-500 hover:text-zinc-300 hover:bg-[#0c0c0f]'
          }">
          <span>${getFileIcon(file.language)}</span>
          <span class="truncate max-w-[120px]">${file.name}</span>
          <button data-action="close" data-tab-id="${file.id}" class="p-0.5 ml-0.5 rounded hover:bg-white/10 text-zinc-500 hover:text-zinc-200 transition-colors">
